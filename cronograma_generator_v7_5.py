...
for task in final_cronograma:
    ...
    # Determine if row is draggable (has checkbox) or fixed (no checkbox)
    draggable_class = "draggable" if checkbox_html else "fixed-row"
    draggable_attr = 'draggable="true"' if checkbox_html else ''
    task_id_attr = f'data-task-id="{task_id}"' if task_id else ''
    duration_attr = f'data-duration="{duration}"'
    
    html_rows += f'''        <tr class="{priority_class} {height_class} {completed_class} {check_class} {draggable_class}" {draggable_attr} {task_id_attr} {duration_attr}>
...
// Función para parsear tiempo "HH:MM" a minutos desde medianoche
...
// Función para recalcular todos los rangos horarios
function recalculateTimeRanges() {{
  ...
  allRows.forEach(row => {{
    ...
    const duration = parseInt(durationAttr) || 0;
    
    // Obtener tiempo de inicio de los spans
    const timeStart = timeBadge.querySelector('.time-start');
    const timeEnd = timeBadge.querySelector('.time-end');
    
    if (!timeStart || !timeEnd) return;
    
    // Si es la primera fila o no tiene tiempo previo, usar el tiempo original
    if (currentTime === null) {{
      const startTimeText = timeStart.textContent;
      currentTime = timeToMinutes(startTimeText);
    }}
    ...
    // Actualizar el DOM
    timeStart.textContent = startTime;
    timeEnd.textContent = endTime;
    
    // Actualizar también los data attributes del checkbox si existe
    const checkbox = row.querySelector('.task-checkbox');
    if (checkbox) {{
      checkbox.setAttribute('data-start-time', startTime);
      checkbox.setAttribute('data-end-time', endTime);
    }}
    
    // Efecto visual de recalculación
    const timeCol = row.querySelector('.time-col');
    if (timeCol) {{
      timeCol.classList.add('time-recalculated');
      setTimeout(() => timeCol.classList.remove('time-recalculated'), 500);
    }}
    
    // Avanzar el tiempo para la siguiente tarea
...
document.addEventListener('DOMContentLoaded', function() {{
  ...
  draggables.forEach(draggable => {{
    ...
    draggable.addEventListener('dragend', function() {{
      ...
      // Recalcular rangos horarios después del drop
    ...
    draggable.addEventListener('drop', function(e) {{
      ...
      if (this !== draggedElement) {{
        ...
        const allRows = Array.from(tbody.querySelectorAll('.draggable'));
...
function saveOrder() {{
  ...
  rows.forEach(row => {{
    ...
    if (taskId && timeBadge) {{
      ...
      order.push({{
        task_id: taskId,
        start_time: timeStart.textContent,
        end_time: timeEnd.textContent
      }});
    }}
  }});
  ...
  console.log('Orden guardado con nuevos horarios:', order);
  ...
  fetch('/save-order', {{
    ...
    if (response.ok) {{
      console.log('✅ Orden guardado en el servidor');
    }}
  }}).catch(error => {{
    console.log('⚠️ No se pudo guardar en el servidor:', error);
  }});
}}
</script>

<!-- Botón flotante para guardar orden -->
<button class="save-button" id="save-button" onclick="saveOrder()">
  💾 Guardar Orden
</button>
