...
def save_task_order():
    """Save the new order of tasks after drag & drop with recalculated times"""
    try:
        data = request.get_json()
        order = data.get('order', [])
        
        # Save to a JSON file for persistence
        save_path = '/app/task_order.json' if os.path.exists('/app') else 'task_order.json'
        
        with open(save_path, 'w') as f:
            json.dump(order, f, indent=2)
        
        print(f"📋 Nuevo orden de tareas guardado con horarios recalculados:")
        for task in order[:5]:  # Print first 5 tasks
            print(f"  - {task['task_id']}: {task['start_time']}-{task['end_time']}")
        if len(order) > 5:
            print(f"  ... y {len(order) - 5} tareas más")
        
        return jsonify({
            'success': True,
            'message': 'Orden guardado correctamente',
            'tasks_saved': len(order)
        })
    
    except Exception as e:
        print(f"Error saving task order: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
